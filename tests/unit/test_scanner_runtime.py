...
def test_evaluate_symbol_lifecycle_boundaries(
    ...
    async def scenario() -> None:
        ...
        cast(Any, service._engine).evaluate_setups = lambda loaded: ([match], [])
        cast(Any, service._engine).shared_entry = lambda e, direction, trigger: entry_ready
        cast(Any, service._engine).score = lambda loaded, accepted, ready: (
            score_result[0],
            score_result[1],
            score_result[2],
            {"trend": Decimal("1")},
...
def test_evaluate_symbol_superseded_and_no_setup() -> None:
    async def scenario() -> None:
        ...
        service._load_context = MethodType(load, service)  # type: ignore[method-assign]
        cast(Any, service._engine).evaluate_setups = lambda loaded: ([first, second], [])
        cast(Any, service._engine).shared_entry = lambda e, d, t: False
        cast(Any, service._engine).score = lambda loaded, accepted, ready: (
        ...
        cast(Any, service._engine).evaluate_setups = lambda loaded: ([], [])
        candidate, audits, _ = await service._evaluate_symbol(universe(), NOW, "run-1")
...
def test_active_refresh_ready_and_failure_paths() -> None:
    async def scenario() -> None:
        ...
        cast(Any, service._engine).invalidated = lambda candidate, s0, e0: False
        cast(Any, service._engine).shared_entry = lambda e, d, t: True
        cast(Any, service._engine).score = lambda ctx, match, entry_ready: (
            90,
            75,
            ScannerGrade.A_PLUS,
            {"setup": Decimal("20")},
